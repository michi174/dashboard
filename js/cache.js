    ...
    loadSummoner(summoner, region) {
        ...
        if (this.summoners.length > 0) {
            ...
            for (let key in this.summoners) {
                let name = this.summoners[key].name
                    .toLowerCase()
                    .replace(/ /g, "");
            ...
            if (_summoner !== null) {
                ...
                console.log(
                    "Cant find " + summoner + " (" + region + ") in Cache"
                );
                return false;
            }
        } else {
    ...
    addSummoner(summoner) {
        if (typeof summoner === "object" && summoner !== null) {
            ...
            for (let [key, oldSummoner] of this.summoners.entries()) {
                if (
                    oldSummoner.accountId === summoner.accountId &&
                    oldSummoner.region === summoner.region
                ) {
                    this.summoners[key] = summoner;
                    newSummoner = false;
                    console.log(
                        "Updated summoner " + summoner.name + " in Cache"
                    );
                    break;
                }
            }
        ...
        if (this.matches.length > 0) {
            for (let [index, match] of this.matches.entries()) {
                ...
                if (parseInt(match.gameId) === parseInt(matchId)) {
                    console.info(
                        "returning match " + match.gameId + " from cache"
                    );
